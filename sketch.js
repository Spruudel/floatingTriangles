...
let BHOLE_ANIM_FRAMES = 30;
let SUN_ANIM_FRAMES = 1;

let ANIM_SPEED = 0.005;
...
function draw() {
	//background(51);
	if (frameCount < 100) image(bHoleAnim[frameCount % bHoleAnim.length], 0, 0);
	/*
...
function mouseClicked() {
	if (keyIsDown(SHIFT)) {
		if (suns.length >= 1)
			suns = [];
		suns.push(new GravObj(createVector(mouseX, mouseY), SUN_R, SUN_COLOR, SUN_ALPHA, SUN_ANGLE_DISTANCE, ANIM_SPEED));
	} else if (!keyIsDown(SHIFT))
		bholes.push(new GravObj(createVector(mouseX, mouseY), BHOLE_R, BHOLE_COLOR, BHOLE_ALPHA, BHOLE_ANGLE_DISTANCE, ANIM_SPEED));
	return false;
}
...
function createAnim(radius, angleDistance, nWaves, color, alpha, animSpeed, nFrames) {
	...
	for (let i = 0; i < nWaves; i++) { // full
		for (let j = 0; j < nAnimParticles; j++) { // one wave
			...
			animParticles.push(new AnimParticle(createVector(floor(radius * 2.5), floor(radius * 2.5)), radius, animSpeed, color, alpha, curAngle, (1 / nWaves) * i));
		}
	}
	...
	for (let i = 0; i < nFrames; i++) {
		let curFrame = createGraphics(radius * 5, radius * 5);
		curFrame.background(220);
